...
c)
Entrambi i rami dell' if then else hanno la stessa complessità. La fase di 
inizializzazione avrà complessità theta(min(n,m)) con n lunghezza di s1 e m 
lunghezza di s2, essendo un ciclo for che esegua min(n,m) iterazioni di 
complessità costante. Sucessivamente avrò due cicli for annidati che in totale eseguiranno n*m iterazione di complessità costante,
quindi complessià theta(n*m). 
La complessità totale sarà quindi theta(n*m).
*/
